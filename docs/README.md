...
| [ADR-003](ADR/ADR-003-desktop-extension-framework.md) | Desktop Extension target framework | ⚠️ Pending on-site verification |
...
| [ADR-006](ADR/ADR-006-rest-api-over-legacy-sdk.md) | REST API V2 over legacy RepositoryAccess SDK | ✅ Accepted |
| [ADR-007](ADR/ADR-007-multi-repository-abstraction.md) | Multi-repository abstraction via IRepositoryContext | ✅ Accepted |

## Deployment (created in Phase 6)
...
## Desktop Extension (created in Phase 5)

| Document | Purpose |
|----------|---------|
| LFDesktopExtension.md | SDK version, .NET target framework, registry key format, deployment path |
